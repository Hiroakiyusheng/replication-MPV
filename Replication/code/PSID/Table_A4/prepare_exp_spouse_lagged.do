...
cd to the folder that stores this code: cd "./Replication/code/PSID/Table_A4"
  ...
  Output: 1. "../../data/PSID/individual_for_exp_nat_lagged_spouse.dta"
          2. "../../data/PSID/individual_for_exp_state_lagged_1_spouse..dta"
		  3. "../../data/PSID/individual_for_exp_personal_lagged_1_spouse..dta"
...
keep if HEAD == 1 | SPOUSE == 1 /* keep only heads and spouses */
keep ID
...
keep if age != 0
sort ID year
saveold "individual_for_exp_nat_lagged_1_spouse.dta", replace version(12)
restore
 
/* create experience measures using t-6 to t-2 years (1 year lag) */
/* get data set for calculating state-level exp */  
  /* Criterion (A) -- 5 consecutive years presence after 1976, the earliest year for which we have state UE data */
drop if year < 1976
...
saveold "individual_for_exp_state_lagged_1_spouse.dta", replace version(12)
...
drop presence tag
replace mark = 0 if mark == .
saveold "individual_for_exp_personal_lagged_1_spouse.dta", replace version(12)

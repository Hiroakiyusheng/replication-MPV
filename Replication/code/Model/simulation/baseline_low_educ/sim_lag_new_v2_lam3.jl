...
include("sim_funs_lag_new_3.jl")

Random.seed!(1234)

function simulate_basic_one(rand_nums, until_50_periods,over_50_periods,retiree_periods,consumption_choices, asset_ind_choices::Array{Int64}, choices, income, asset_grid, u_transition, a_weights;verbose=false)
    ...
    # we don't use last random a so instead we use it here
    cur_a = map_a(rand_nums[pre_retirement_periods,rand_a_index])

    state_array[1] = cur_state
    asset_ind_array[1] = cur_assets
    cons_array[1] = consumption_choices[1,cur_assets,cur_u,cur_state,cur_a]
    inc_array[1] = income[1,cur_assets,cur_u,cur_state,cur_a]
    decision = choices[1,cur_assets,cur_u,cur_state,cur_a]


    # get assets
    cur_assets = asset_ind_choices[1,cur_assets,cur_u,cur_state,cur_a]
    ...
    for i = 2:until_50_periods
        ...
        cons_array[i] = consumption_choices[i,cur_assets,cur_u,cur_state,cur_a]
        inc_array[i] = income[i,cur_assets,cur_u,cur_state,cur_a]

        # get assets
        cur_assets = asset_ind_choices[i,cur_assets,cur_u,cur_state,cur_a]
        ...
        if cur_state == working_state
            # get decision
            decision = choices[i,asset_ind_array[i],cur_u,cur_state,cur_a]
...
println("file read time")
@time con, ass, assi, val, choice, inc = load("solve_mats_delta_049_lam_e_67_lam_n_76.jld2", "con", "ass", "assi", "val", "choice", "inc")
...
@time for i = 1:N
           ...
           state_array, asset_ind_array, cons_array,inc_array, work_array, delta_array = simulate_basic_one(rand_nums,until_50_periods,over_50_periods,retiree_periods,con, assi, choice, inc, asset_grid, u_transition, a_weights)
           ...
           ass_all[i,:] = act_ass
           p_delta_all[i,:] = get_p_delta_path(delta_array,work_array,delta)
           avg_delta_all[i,:] = get_avg_delta_path(delta_array,work_array,delta)
           ...
           # println(dis_min)
           # println(maximum(state_array))
           if dis_min < 161
               if dis_min < 159
                   # println(argmax(state_array))
                   # println("hey")
                   global meanval = (countval * meanval + dis_min) / (countval + 1)
                   global countval = countval + 1
               end
           end
           if i == N
               println("mean:", meanval)
               println("count: ",countval)
           end
end
...
CSV.write("../../../../data/simulations/straight_pistaferri/low_educ/reg_data_3.csv",out_df)

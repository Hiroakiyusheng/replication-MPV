...
# equal weights
function perceived_delta(delta_array,work_array,i,true_delta;lambda=1)
    if i < 3
        return true_delta
    end
...
function average_delta(delta_array,work_array,i,true_delta;lambda=0)
    if i < 3
        return true_delta
    end
    # generate unit weights
...
function get_avg_delta_path(delta_array,work_array,true_delta;last_period=200)
    ...
    for i = 3:last_period
        if sum(work_array[1:(i-2)]) < .5
            ...
            out_array[i] = average_delta(delta_array[1:i],work_array[1:i],i,true_delta)
        end
    end
    if sum(@. isnan(out_array)) > 0
        println(delta_array)

...
function solve_last_period(asset_grid,u_grid)
    ...
    # Set up array
    value_array = Array{Float64}(undef,u_size,asset_size)
...
function solve_retirement(asset_grid,u_grid,retiree_periods)
    ...
    # set up arrays
    consumption_choice = SharedArray{Float64}(u_size,asset_size,retiree_periods)
    next_asset_choice = SharedArray{Float64}(u_size,asset_size,retiree_periods)
    next_asset_index = SharedArray{Int64}(u_size,asset_size,retiree_periods)
    value_array = SharedArray{Float64}(u_size,asset_size,retiree_periods)
    income_array = SharedArray{Float64}(u_size,asset_size,retiree_periods)
    ...
    for i = (retiree_periods - 1):-1:1
        ...
        next_period = i+1
        @views period_values = value_array[:,:,next_period]
        @sync @distributed for j = 1:asset_size
            ...
            for k = 1:u_size
                ...
                consumption_choice[k,j,i] = cons
                next_asset_choice[k,j,i] = next_asset
                next_asset_index[k,j,i] = next_asset_ind
                value_array[k,j,i] = value
...
function solve_over_50(delta,lambda_e,lambda_n,asset_grid,u_grid,u_transition,a_grid,a_weights,
    ...
    # set up arrays
    consumption_choice = SharedArray{Float64}(a_size,state_count,u_size,asset_size,over_50_periods)
    next_asset_choice = SharedArray{Float64}(a_size,state_count,u_size,asset_size,over_50_periods)
    next_asset_index = SharedArray{Int64}(a_size,state_count,u_size,asset_size,over_50_periods)
    value_array = SharedArray{Float64}(a_size,state_count,u_size,asset_size,over_50_periods)
    realized_income_array = SharedArray{Float64}(a_size,state_count,u_size,asset_size,over_50_periods)
    choice_array = SharedArray{Float64}(a_size,state_count,u_size,asset_size,over_50_periods)
    # Choices
    # 0 - N/A
    # 1 - Work
    # 2 - Don't work
    ...
    income_array = Array{Float64}(undef,a_size,u_size,tot_years)
    unemployment_array = Array{Float64}(undef,a_size,u_size,tot_years)
            ...
            for k = 1:state_count
                ...
                if k == 1
                    ...
                    for m = 1: a_size
                        ...
                        working_income = income_array[m,j,tot_years]
                        ...
                        #   We need not do k = 2
                        consumption_choice[m,k,j,i,over_50_periods] = cons
                        next_asset_choice[m,k,j,i,over_50_periods] =next_asset
                        next_asset_index[m,k,j,i,over_50_periods] = next_asset_ind
                        value_array[m,k,j,i,over_50_periods] = value
                        choice_array[m,k,j,i,over_50_periods] = work_choice
                        realized_income_array[m,k,j,i,over_50_periods] = realized_income
                    end
                elseif k == 2
                    # Taken care of by identical case 1
                    for m = 1: a_size
                        consumption_choice[m,k,j,i,over_50_periods] = consumption_choice[m,working_state,j,i,over_50_periods]
                        next_asset_choice[m,k,j,i,over_50_periods] = next_asset_choice[m,working_state,j,i,over_50_periods]
                        next_asset_index[m,k,j,i,over_50_periods] = next_asset_index[m,working_state,j,i,over_50_periods]
                        value_array[m,k,j,i,over_50_periods] = value_array[m,working_state,j,i,over_50_periods]
                        choice_array[m,k,j,i,over_50_periods] = choice_array[m,working_state,j,i,over_50_periods]
                        realized_income_array[m,k,j,i,over_50_periods] = realized_income_array[m,working_state,j,i,over_50_periods]
                    end
                elseif k == 3
                    # Must consider all job fit possibilities
                    for m = 1: a_size
                        ...
                        income = unemployment_array[m,j,tot_years]
                        ...
                        next_asset_index[m,k,j,i,over_50_periods] = next_asset_ind
                        value_array[m,k,j,i,over_50_periods] = value
                        realized_income_array[m,k,j,i,over_50_periods] = income
                    end
                elseif k == 4
                    # we assume no stochastic shocks in the last period
                    next_values = next_value_array_over50[j,:]
                    ...
                    @. consumption_choice[:,k,j,i,over_50_periods] = cons
                    @. next_asset_choice[:,k,j,i,over_50_periods] = next_asset
                    @. next_asset_index[:,k,j,i,over_50_periods] = next_asset_ind
                    @. value_array[:,k,j,i,over_50_periods] = value
                    @. realized_income_array[:,k,j,i,over_50_periods] = income
    ...
    # next_value_array = SharedArray{Float64}(asset_size,u_size,state_count,a_size)
    # solve for all remaining periods in this bin
    for cur_period = (over_50_periods - 1):-1:1
        # @everywhere GC.gc()
        cur_period_full = 112 + cur_period
        println("Period: ",cur_period_full)
        # get the age
        approx_age1 = cur_period / 4
        approx_age2 = floor(approx_age1)
        approx_age3 = approx_age2 + 1
        cur_year = Int(approx_age3)
        # get next values
        @views next_value_array1 = value_array[:,:,:,:,(cur_period + 1)]
        # Hypothesis here is that solve states is faster with opposite indexing
        next_value_array = permutedims(next_value_array1,[4,3,2,1])
        # @time next_value_array = SArray{Tuple{asset_size,u_size,state_count,a_size},Float64}(next_value_array_init)
                ...
                @simd for k = 1:state_count
                    ...
                    if k == 1
                        ...
                        @simd for m = 1:a_size
                            ...
                            working_income = income_array[m,j,cur_year]
                            value, cons, next_asset, next_asset_ind, realized_income, work_choice =
                            ...
                            consumption_choice[m,k,j,i,cur_period] = cons
                            next_asset_choice[m,k,j,i,cur_period] =next_asset
                            next_asset_index[m,k,j,i,cur_period] = next_asset_ind
                            value_array[m,k,j,i,cur_period] = value
                            choice_array[m,k,j,i,cur_period] = work_choice
                            realized_income_array[m,k,j,i,cur_period] = realized_income
                        end
                    elseif k == 2
                        # Must consider all job fit possibilities
                        @simd for m = 1: a_size
                    ...
                    elseif k == 3
                        ...
                        @simd for m = 1: a_size
                            wait_for_job_values = get_dont_work_values(j, next_value_array,lambda_n,
                                                    working_state,unemployed_di_elig_state,u_transition,a_weights)
                            income = unemployment_array[m,j,cur_year]
                            ...
                            next_asset_index[m,k,j,i,cur_period] = next_asset_ind
                            value_array[m,k,j,i,cur_period] = value
                            choice_array[m,k,j,i,cur_period] = di_choice
                            realized_income_array[m,k,j,i,cur_period] = income
                        end
                    elseif k == 4
                        # we assume no stochastic shocks in the last period
                        wait_for_job_values = get_dont_work_values(j, next_value_array,lambda_n,
                        ...
                        @. consumption_choice[:,k,j,i,cur_period] = cons
                        @. next_asset_choice[:,k,j,i,cur_period] = next_asset
                        @. next_asset_index[:,k,j,i,cur_period] = next_asset_ind
                        @. value_array[:,k,j,i,cur_period] = value
                        @. choice_array[:,k,j,i,cur_period] = di_choice
                        @. realized_income_array[:,k,j,i,cur_period] = income
                    ...
                    elseif k == 6
                        # we assume no stochastic shocks in the last period
                        # Note indexing on next values is reversed
...
function solve_until_50(delta,lambda_e,lambda_n,asset_grid,u_grid,u_transition,a_grid,a_weights,
    ...
    # set up arrays
    consumption_choice = SharedArray{Float64}(a_size,state_count,u_size,asset_size,until_50_periods)
    next_asset_choice = SharedArray{Float64}(a_size,state_count,u_size,asset_size,until_50_periods)
    next_asset_index = SharedArray{Int64}(a_size,state_count,u_size,asset_size,until_50_periods)
    value_array = SharedArray{Float64}(a_size,state_count,u_size,asset_size,until_50_periods)
    realized_income_array = SharedArray{Float64}(a_size,state_count,u_size,asset_size,until_50_periods)
    choice_array = SharedArray{Float64}(a_size,state_count,u_size,asset_size,until_50_periods)
    # Choices
    # 0 - N/A
    # 1 - Work
    # 2 - Don't work
    ...
    # !!!!!!!!!!!!!!!!! BE sure dims on next_value_array_until50 are permuted correctly
    #  ---> currently this is handled by overarching function, solve_all
    @sync @distributed for i = 1:asset_size
        for j = 1:u_size
            # Now we solve for each state
            for k = 1:state_count
                ...
                if k == 1
                    ...
                    for m = 1: a_size
                        ...
                        # Note that since DI eligibility is null in the last period
                        #   We need not do k = 2
                        consumption_choice[m,k,j,i,until_50_periods] = cons
                        next_asset_choice[m,k,j,i,until_50_periods] =next_asset
                        next_asset_index[m,k,j,i,until_50_periods] = next_asset_ind
                        value_array[m,k,j,i,until_50_periods] = value
                        choice_array[m,k,j,i,until_50_periods] = work_choice
                        realized_income_array[m,k,j,i,until_50_periods] = realized_income
                    end
                elseif k == 3
                    # Must consider all job fit possibilities
                    for m = 1: a_size
                        ...
                        next_asset_index[m,k,j,i,until_50_periods] = next_asset_ind
                        value_array[m,k,j,i,until_50_periods] = value
                        realized_income_array[m,k,j,i,until_50_periods] = income
                    end
                elseif k == 4
                    # # we assume no stochastic shocks in the last period
                    next_values = get_dont_work_values(j, next_value_array_until50,lambda_n,
                    ...
                    @. consumption_choice[:,k,j,i,until_50_periods] = cons
                    @. next_asset_choice[:,k,j,i,until_50_periods] = next_asset
                    @. next_asset_index[:,k,j,i,until_50_periods] = next_asset_ind
                    @. value_array[:,k,j,i,until_50_periods] = value
                    @. realized_income_array[:,k,j,i,until_50_periods] = income
                else
                    nothing
                    # not all states are represented here
                end
            end
        end
    end
...
function solve_all(delta,lambda_e,lambda_n,asset_grid,u_grid,u_transition,a_grid,
    ...
    con_retired_f = Array{Float64}(undef,a_size,state_count,s[1],s[2],s[3])
    ass_retired_f = Array{Float64}(undef,a_size,state_count,s[1],s[2],s[3])
    assi_retired_f = Array{Int64}(undef,a_size,state_count,s[1],s[2],s[3])
    val_retired_f = Array{Float64}(undef,a_size,state_count,s[1],s[2],s[3])
    inc_retired_f = Array{Float64}(undef,a_size,state_count,s[1],s[2],s[3])
    ...
    for i = 1:s[3]
        ...
        for j = 1:s[2]
            # u value
            for k = 1:s[1]
                @. con_retired_f[:,:,k,j,i] = con_retired[k,j,i]
                @. ass_retired_f[:,:,k,j,i] = ass_retired[k,j,i]
                @. assi_retired_f[:,:,k,j,i] = assi_retired[k,j,i]
                @. val_retired_f[:,:,k,j,i] = val_retired[k,j,i]
                @. inc_retired_f[:,:,k,j,i] = inc_retired[k,j,i]
            end
        end
    end
    ...
    @everywhere GC.gc()
    @views next_value_array2_raw = val_over50[:,:,:,:,1]
    next_value_array2 = permutedims(next_value_array2_raw,[4,3,2,1])
    ...
    con_all = cat(con_under50, con_over50, con_retired_f;dims=5)
    ass_all = cat(ass_under50, ass_over50, ass_retired_f;dims=5)
    assi_all = cat(assi_under50, assi_over50, assi_retired_f;dims=5)
    val_all = cat(val_under50, val_over50,val_retired_f;dims=5)
    inc_all = cat(inc_under50, inc_over50,inc_retired_f;dims=5)
    choice_all = cat(choice_under50,choice_over50;dims=5)

    con_all = permutedims(con_all,[5,4,3,2,1])
    ass_all = permutedims(ass_all,[5,4,3,2,1])
    assi_all = permutedims(assi_all,[5,4,3,2,1])
    val_all = permutedims(val_all,[5,4,3,2,1])
    inc_all = permutedims(inc_all,[5,4,3,2,1])
    choice_all = permutedims(choice_all,[5,4,3,2,1])
...
function solve_save_delta_grid(delta_grid,lambda_e,lambda_n,asset_grid,u_grid,u_transition,a_grid,
    ...
    con_io = open("con.bin","w+")
    ass_io = open("ass.bin","w+")
    assi_io = open("assi.bin","w+")
    val_io = open("val.bin","w+")
    choice_io = open("choice.bin","w+")
    inc_io = open("inc.bin","w+")
...
function save_solve_all_output(delta,lambda_e,lambda_n,con, ass, assi, val, choice, inc)
    tag = string("./hold/solve_mats_delta_", delta, "_lam_e_", lambda_e, "_lam_n_", lambda_n)
    tag = replace(tag,"0." => "")
...
function save_solve_dg_output(delta_grid,lambda_e,lambda_n,con, ass, assi, val, choice, inc)
    ...
    tag = string("solve_",minimum(delta_grid),"_delta_grid_", dg_size, "_lam_e_", lambda_e, "_lam_n_", lambda_n)
    tag = replace(tag,"0." => "")
    file_name = string(tag,".jld2")
    println(file_name)
...
#     delta_size = sizeof(delta_grid)
#     tag = string("solve_mats_delta_size_", delta_size, "_lam_e_", lambda_e, "_lam_n_", lambda_n)
#     tag = replace(tag,"0." => "")
#     file_name = string(tag,".jld2")
#     println(file_name)
...
# Profile.init(n = 10^9,delay=1.0)
# @profile solve_all(delta,lambda_e,lambda_n,asset_grid,u_grid,u_transition,a_grid,
#                         a_weights,until_50_periods,
#                         over_50_periods,retiree_periods)
# Profile.print()
# ProfileView.view()
# Juno.profiletree()
# Juno.profiler()
# @profiler solve_all(delta,lambda_e,lambda_n,asset_grid,u_grid,u_transition,a_grid,
#                         a_weights,until_50_periods,
#                         over_50_periods,retiree_periods)

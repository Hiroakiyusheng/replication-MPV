...
foreach y in "1" "3" {
	foreach w in "straight_pistaferri" "consumption_scarring" {
		foreach x in "reg_data_" "reg_data-beh_" {
...
//			log variables
			...
			reghdfe consumption income  p_delta, absorb(period educ) cluster(period)
			...
			reghdfe consumption assets  income  p_delta, absorb(period educ) cluster(period)
			estadd local lambda "`y'"
			estadd local B_or_R "$learning_type"
			estadd local educ_lvl "$education_level"
			estadd local cons_s   "$consumption_scarring"	
...
esttab using "../../Tables/table_7.csv",  replace label b(%8.3f) t  nocon nostar ///
order(income assets p_delta) ///
s(lambda B_or_R cons_s C_or_Reg P_FE r2 N, label("Lambda" "Rational or EBL" "Model" "Period Clustered SE" "Period (Age) FE" "R2"))

esttab using "../../Tables/table_7.tex", replace label nocon nostar ///
order(income assets p_delta) ///
